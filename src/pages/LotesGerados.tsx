...
import { mockLotes, categoriasLabels, estadoLabels, statusLabels } from "@/data/mockData";
import type { Lote } from "@/data/mockData";
...
const LotesGerados = () => {
  ...
  const [expanded, setExpanded] = useState<string | null>(mockLotes[0]?.id);
  ...
  const aprovarLote = (id: string) => {
    setLotes((prev) =>
      prev.map((l) =>
        l.id === id ? { ...l, status: "aprovado", precoAprovado: l.precoSugerido } : l
      )
    );
  ...
  const startEditPrice = (lote: Lote) => {
    ...
    setEditValue(lote.precoSugerido.toString());
  };
  ...
  const savePrice = (id: string) => {
    ...
    if (!isNaN(numValue) && numValue > 0) {
      setLotes((prev) =>
        prev.map((l) => (l.id === id ? { ...l, precoSugerido: numValue } : l))
      );
  ...
  const aprovarTodos = () => {
    setLotes((prev) =>
      prev.map((l) => ({ ...l, status: "aprovado", precoAprovado: l.precoSugerido }))
    );
    toast.success("Todos os lotes aprovados!");
  };

  const totalEstimado = lotes.reduce((sum, l) => sum + l.precoSugerido, 0);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Revise os lotes formados pela IA e aprove os preços sugeridos
          </p>
        </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card p-4 flex flex-wrap items-center gap-6">
        ...
        <div>
          ...
          <p className="text-xl font-display font-bold text-foreground">
            {lotes.reduce((sum, l) => sum + l.itens.length, 0)}
          </p>
        </div>
        <div className="w-px h-10 bg-border" />
        <div>
            ...
            >
              ...
              >
                <div className={cn(
                  ...
                  isApproved ? "bg-success/10" : "bg-accent/10"
                )}>
                ...
                <div className="text-right mr-4">
                  ...
                  {editingPrice === lote.id ? (
                    ...
                    <p className="text-lg font-display font-bold text-foreground">
                      {lote.precoSugerido.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
